...
namespace Silverton.Core.IO {
#nullable enable

    // Asynchronous stream reader, useful for listening to pipes
    // Based on https://github.com/dotnet/runtime/blob/main/src/libraries/System.Diagnostics.Process/src/System/Diagnostics/AsyncStreamReader.cs
        ...
        private async Task ReadBufferAsync() {
            ...
            while (true) {
                try {
                    ...
                    int bytesRead = await _stream.ReadAsync(_byteBuffer, 0, _byteBuffer.Length).ConfigureAwait(false);
                    if (bytesRead == 0)
                        break;
